...
public class FileAppenderIT extends DocFactoryTest {
	...
	public void filterPdf() {
		List<File> filesToFilter = Arrays.asList(new File("aTxt.txt"), new File("aPdf.pdf"), new File("aDocx.docx"), 
	...
	public void filterOffice() {
		List<File> filesToFilter = Arrays.asList(new File("aTxt.txt"), new File("anOdt.ODT"), new File("aDocx.docx"), 
	...
	public void appendOfficeFiles() throws Exception {
		...
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/document1.doc").toURI().getPath()));
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/document2.docx").toURI().getPath()));
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/document3.pdf").toURI().getPath())); // will be ignored
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/another_doc.doc").toURI().getPath()));
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/another_odt.odt").toURI().getPath()));
	...
	public void appendPdfFiles() throws Exception {
		List<File> filesToAppend = new ArrayList<>();
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/pdf1.pdf").toURI().getPath()));
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/document2.docx").toURI().getPath())); // will be ignored
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/document3.pdf").toURI().getPath()));
		filesToAppend.add(new File(FileAppenderIT.class.getResource("resources/files/pdf2.pdf").toURI().getPath()));
		...
		File result = FileAppender.getInstance().appendPdfFiles(filesToAppend, 
				...
				// .withAppendedFileOutputFormat(DocFactoryConstants.DOCX_FORMAT) setting the format has no effect, only pdf output will be done
				// .useHeadersFootersFromLeadingPage(true) or restartPageNumbering(true) has no effect
				);
