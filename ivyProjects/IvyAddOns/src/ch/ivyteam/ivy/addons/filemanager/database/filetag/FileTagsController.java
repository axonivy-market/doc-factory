...
public class FileTagsController extends AbstractFileTagsController {
	...
	protected FileTagsController(IFileTagPersistence ftPersistence) {
		this.ftPersistence = ftPersistence;
	}
	...
	@Override
	public FileTag updateFileTag(long fileId, String oldTag, String newTag)
			throws Exception {
		if(fileId <= 0) {
			throw new IllegalArgumentException("The FileId must be greater than zero.");
		}
		API.checkNotEmpty(oldTag, "Old String value of the tag");
		API.checkNotEmpty(newTag, "New String value of the tag");
		
		FileTag fileTag = getTag(fileId, oldTag);
		if(fileTag == null) {
			return null;
		}
		fileTag.setTag(newTag); 
		return this.ftPersistence.update(fileTag);
	}
