		...
		Object[] ret = null;
			...
			if(rdPanel!=null && methodName !=null && methodName.trim().length()>0){
				...
				try {
					...
					Ivy.log().warn("InvocationTargetException in RDCallbackMethodHandler (callRDMethod) called on IRichDialogPanel "+rdPanel.getName()+" on following callback method "+methodName+" with following parameters "+s,_ex);
					...
					//only warn, because this Error is in general generated by a Synchronous Message in the Rich Dialog called method.
					Ivy.log().warn("Warn in RDCallbackMethodHandler (callRDMethod) called on IRichDialogPanel "+rdPanel.getName()+" on following callback method "+methodName+" ,"+_ex.getMessage());
				}
			}else{
		...
		if(rdPanel!=null && methodName !=null && methodName.trim().length()>0){
			if(!(rdPanel instanceof IRichDialogPanel)) {
				...
				try {
					...
					//Just warn because we call "automatically" some callback methods that does not exist in every RDC.
					Ivy.log().warn("InvocationTargetException in RDCallbackMethodHandler (callRDMethodFromULCComponent) called on ULCComponent "+rdPanel.getName()+" on following callback method "+methodName+" ,"+_ex.getMessage());
				} catch (NoSuchMethodException _ex) {
					//Just warn because we call "automatically" some callback methods that does not exist in every RDC.
					Ivy.log().warn("NoSuchMethodException in RDCallbackMethodHandler (callRDMethodFromULCComponent) called on ULCComponent "+rdPanel.getName()+" on following callback method "+methodName+" ,"+_ex.getMessage());
				}catch(Exception _ex){
					...
					//only warn, because this Error is in general generated by a Synchronous Message in the Rich Dialog called method.
					Ivy.log().warn("Warn in RDCallbackMethodHandler (callRDMethodFromULCComponent) called on ULCComponent "+rdPanel.getName()+" on following callback method "+methodName+" ,"+_ex.getMessage());
				}
			}
		}
		return ret;
