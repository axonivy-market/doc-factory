		...
		Object[] ret = null;
			...
			if(rdPanel!=null && methodName !=null && methodName.trim().length()>0){
				try {
					...
					Ivy.log().warn("Error in RDCallbackMethodHandler (callRDMethod) called on IRichDialogPanel "+rdPanel.getName()+" on following callback method "+methodName+" ,"+_ex.getMessage());
				}
			}else{
		...
		if(rdPanel!=null && methodName !=null && methodName.trim().length()>0){
			if(!(rdPanel instanceof IRichDialogPanel)) {
				...
				try {
					...
					//only warn, because this Error is in general generated by a Synchronous Message in the Rich Dialog called method.
					Ivy.log().warn("Error in RDCallbackMethodHandler (callRDMethodFromULCComponent) called on ULCComponent "+rdPanel.getName()+" on following callback method "+methodName+" ,"+_ex.getMessage());
				}
			}
		}
		return ret;
