...
public class MailMergeDataSourceGenerator {
	...
	public static IMailMergeDataSource getFromCollectionTypeTemplateMergeField(TemplateMergeField templateMergeField) {
		...
		if(templateMergeField.getType().is(TemplateMergeFieldType.COLLECTION)) {
			...
			mmds = new MailMergeDataSource(serializableMailMergeDataSource.recordset, templateMergeField.getMergeFieldName());
	...
	private static IMailMergeDataSource getFromCollectionTypeTemplateMergeField(TemplateMergeField templateMergeField, String parent) {
		...
		if(templateMergeField.getType().is(TemplateMergeFieldType.COLLECTION)) {
			...
			mmds = new MailMergeDataSource(serializableMailMergeDataSource.recordset, getLastPart(parent) + getSecondPart(templateMergeField.getMergeFieldName()));
			Collection<TemplateMergeField> collectionFieldValue = templateMergeField.getChildren();
			for(TemplateMergeField tmf: collectionFieldValue) {
				if(tmf.isCollection() || tmf.getType().is(TemplateMergeFieldType.OBJECT)) {
					makeEmbeddedMailMergeDataSources(mmds, tmf, serializableMailMergeDataSource.tableName);
	...
	private static IMailMergeDataSource makeEmbeddedMailMergeDataSources(IMailMergeDataSource parentMmds, TemplateMergeField templateMergeField, String parentTableName) {
		...
		if(templateMergeField.getType().is(TemplateMergeFieldType.OBJECT)) {
			...
			for(TemplateMergeField tmf : childrenFields) {
				...
				if(result != null) {
					parentDataSource.putChildMailMergeDataSource(result, simplifiedName + getSecondPart(tmf.getMergeFieldName()));
				}
			}
		}
	...
	private void buildRecordset() {
		Collection<Collection<TemplateMergeField>> mergeFieldsCollections = MergeFieldsExtractor.getMergeFieldsForCollectionTemplateMergeField(tableName, templateMergeField);
