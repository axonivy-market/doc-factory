...
{ 
	...
	private void initialize()
	{
		try {
			if(!RichDialogUtil.isRunningInVisualEditor()) {
				this.xpertlineLF = Ivy.wf().getSecurityContext().executeAsSystemUser(new Callable<Boolean>() {
					@Override
			...
			Ivy.log().error("Exception occurred while getting the ria look and feel {0}",e);
		}
	...
	private RButton getFileManagementButton() {
		if (fileManagementButton == null) {
			...
			this.fixXpertlineLookAndFeel(fileManagementButton);
		}
	...
	private void fixXpertlineLookAndFeel(ULCAbstractButton co){
		if(this.xpertlineLF) {
			co.setText("<html><font color=\"white\">"+co.getText().replace("<html>", "").replace("</html>", "")+"</font></html>");
		}
	}
