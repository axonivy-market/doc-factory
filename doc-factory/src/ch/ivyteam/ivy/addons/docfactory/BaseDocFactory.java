...
/**
 * @author ec This is the abstract class that every DocFactory class should
 *         subclass<br>
 *         You have the possibility to communicate the results, errors, progress
 *         of the documents generation methods<br>
...
public abstract class BaseDocFactory {
  ...
  /**
   * the document factory use a document model like an office dot template to
   * generate serial letters.<br>
   * This is the template java.io.File Object to be used for this operation.
   */
  protected File template;
  /**
   * This Object is used to gives the RD Panel the result of the documents
   * generation operation.<br>
   * This fileOperationMessage contains a type indicating if the operation was
   * successful or not, and a List of generated Files.<br>
   ...
   */
  public abstract FileOperationMessage generateBlankDocument(String outputName, String outPath,
          String outFormat);
   ...
   * @param outPath : where to save the generated file on the server
   * @param outFormat : format ".doc", ".docx", ".pdf", or ".html"
   ...
   *         the java.io.File generated in case of success, else null.<br>
   */
  public abstract FileOperationMessage generateDocument(String templatePath, String fileName,
          String outPath, String outFormat, List<TemplateMergeField> list);
   ...
   *         the java.io.File generated in case of success, else null.<br>
   *
   *         public abstract fileOperationMessage
   *         generateDocument(List<TemplateMergeField> list);
   */
  ...
  /**
   * Method to generate a list from Files that can be used for HTML
   * representation.<br>
   * This this typical if you use a template with Objects like Images in it.<br>
   * During the html File Generation, each image will be saved into a jpg or gif
   * File.<br>
   * If you want to correctly get all the Files involved into the HTML
   * rendering, you have to get the list of images also.
   * @param templatePath : where to find the template
   * @param list : List of parameters (TemplateMergeField objects).
   * @return the HTML and media Files that were generated to represent an HTML
   *         page.
   */
  public abstract ArrayList<java.io.File> generateFilesForHTML(String templatePath,
          List<TemplateMergeField> list);

  /**
   * Method to generate one or more documents<br>
   * with a list of DocumentTemplates Objects containing all the necessary
   * variables for this operation.<br>
   ...
   * @param templatePath : where to find the template
   * @param outPath : where to save the generated File on the server
   * @param outFormat : format ".doc", ".docx", ".pdf", ".txt" or ".html"
   ...
   *         the text of the message and null File Object<br>
   */
  public abstract FileOperationMessage generateDocuments(String templatePath, String outPath,
          String outFormat, List<List<TemplateMergeField>> list);
   ...
   *          the fields merging.
   * @param outputName optional String for the name of the created document
   * @param outPath optional String for the destination folder path
   * @param outFormat format ".doc", ".docx", ".pdf", ".txt" or ".html"
   * @param mergefields a List of
   *          ch.ivyteam.ivy.addons.docfactory.TemplateMergeField Objects for
   *          "normal" mail merge.
   * @param tablesNamesAndFieldsmap HashMap<String, List<CompositeObject>>
   *          HashMap containing the tables names (merge regions) from the
   *          template an the Lists of dataClasses Objects to feed the tables.
   * @return The fileOperationMessage object containing the Type of the message
   *         (FileHandler.SUCCESS, ERROR, INFORMATION_MESSAGE),<br>
   *         the text of the message and null File Object<br>
   */
  public abstract FileOperationMessage generateDocumentWithRegions(String templatePath, String outputName,
   ...
   *          ch.ivyteam.ivy.addons.docfactory.TemplateMergeField Objects for
   *          "normal" mail merge.
   * @param hashtable Hashtable<String, Recordset> each String is the table Name
   *          in the template with its corresponding Recordset to feed the
   *          table. <br>
   *          Each recordset column name should correspond to a merge region
   *          field name in its corresponding table.
   * @return The fileOperationMessage object containing the Type of the message
   *         (FileHandler.SUCCESS, ERROR, INFORMATION_MESSAGE),<br>
   *         the text of the message and null File Object<br>
   */
   ...
   *          ch.ivyteam.ivy.addons.docfactory.TemplateMergeField Objects for
   *          "normal" mail merge.
   * @param tablesNames list of String representing the table names in the
   *          template
   * @param tables_fieldsNames for each table a list of String representing each
   *          merge field with region name (column name)
   * @param tables_rowsValues for each table a list of list of values (list of
   *          rows) to feed the corresponding table.
   * @return The fileOperationMessage object containing the Type of the message
   *         (FileHandler.SUCCESS, ERROR, INFORMATION_MESSAGE),<br>
   *         the text of the message and null File Object<br>
   */
  ...
  public abstract FileOperationMessage generateDocumentWithNestedRegions(String templatePath,
          String outputName, String outPath, String outFormat, List<TemplateMergeField> mergefields,
   ...
   * Method to generate one or more documents each one can be saved in a
   * different destination folder.<br>
   * Each DocumentTemplate Object represents a letter that should be generated.
   * It contains the fileName, its format, path, and all the
   * TemplateMergefields.
   * @param list : List of List of DocumentTemplate. <br>
   *          Each List of TemplateMergeField objects in the primary List will
   *          be turned into a new document.
   * @return The List of fileOperationMessage objects (one for each
   *         DocumentTemplate) containing the Type of the message
   *         (FileHandler.SUCCESS, ERROR, INFORMATION_MESSAGE),<br>
   *         the text of the message and File Object<br>
   * @see DocumentTemplate
   */
  public abstract List<FileOperationMessage> generateDocumentsWithDifferentDestination(
   ...
   *
   *         public abstract fileOperationMessage
   *         generateMultipleDocumentsInOne(String outputName, String
   *         outputPath, String outputFormat,List<List<TemplateMergeField>>
   *         list);
   */

  /**
   * Method to generate multiple documents appended in one File
   * @param templatePath : the path leading to the template (document model)
   *          used to produce the letters with mail merging
   ...
   *         the java.io.File generated in case of success, else null.<br>
   */
  public abstract FileOperationMessage generateMultipleDocumentsInOne(String templatePath, String outputName,
          String outPath, String outFormat, List<List<TemplateMergeField>> list);

  /**
   * Method to generate multiple documents appended in one File<br>
   * Here the template path has to be indicated into the documentTemplate
   * objects
   * @param outPath : where to save the new generated File on the server (can
   *          have a default like "ivy_RIA_files")
   * @param outputName : the desired Name for the document (can have a default
   *          like "letter_nanotime")
   * @param outFormat : format ".doc" (can be the default), ".docx", ....
   * @param list : List DocumentTemplate Objects. Each one contains the
   *          necessary variables for its output.
   * @return The fileOperationMessage object containing the Type of the message
   *         (FileHandler.SUCCESS, ERROR, INFORMATION_MESSAGE),<br>
   *         the text of the message and <br>
   *         the java.io.File generated in case of success, else null.<br>
   * @see DocumentTemplate
   */
  public abstract FileOperationMessage generateMultipleDocumentsInOne(String outPath, String outputName,
          String outFormat, List<DocumentTemplate> list);
  ...
  /**
   * This method should check if the format denoted by the String param is
   * supported by your document Factory
   * @param _format
  ...
  /**
   * get the basic File name that is going to be used to name the generated
   * documents
   * @return the basic filename used to generate multiple documents. If null or
   *         blank, the name of the file should be given as parameter
   */
  ...
  /**
   * set the basicfilename used to generate multiple documents. If null or
   * blank, the name of the file should be given as parameter
   * @param basicFileName
   */
  ...
  /**
   * This method allows getting an instance of the actual document factory that
   * is implemented. If a System property "document.factory" is set, then it
   * will try to return a new Instance of this factory object.<br>
   * This system property must contain the full namespace of the document
   * factory class.
  ...
  public static BaseDocFactory getInstance() {
    try {
      if (StringUtils.isBlank(System.getProperty(DOCUMENT_FACTORY_IMPLEMENTATION_SYSTEM_PROPERTY))) {
        ...
        return (BaseDocFactory) Class.forName(System.getProperty("document.factory")).getDeclaredConstructor()
                .newInstance();
      }
    } catch (Exception e) {
  ...
  /**
   * Set the response handler. A ResponseHandler must implement the
   * handleDocFactoryResponse method accepting a {@link FileOperationMessage}
   * object.
   * @param handler
   */
  public BaseDocFactory withResponseHandler(ResponseHandler handler) {
   ...
   * {@link ch.ivyteam.ivy.addons.docfactory.aspose.AsposeFieldMergingCallback}
   * for Aspose.<br>
   * <b>Note:</b> at the time of the writing of this method, the default
   * DocFactory is based on Aspose. As there is already a default one, You don't
   * need to set the fieldMergingCallback unless you really need it.
   * @param callback
   * @return the baseDocFactory with the given fieldMergingCallback set.
   */
  public <T> BaseDocFactory withFieldMergingCallBack(T callback) {
    API.checkNotNull(callback, "callback");
    this.fieldMergingCallback = callback;
    return this;
  }
   ...
   * some custom logic on the document before or after mail merging and before
   * it has been returned to the user. <b>Note:</b> at the time of the writing
   * of this method, the default DocFactory is based on Aspose. A DocumentWorker
   * Interface has been made for the AsposeDocFactory. See:
   * {@link ch.ivyteam.ivy.addons.docfactory.aspose.DocumentWorker} <br>
   * The AsposeDocFactory implements also its own
   * {@link AsposeDocFactory#withDocumentWorker(DocumentWorker)} method, in the
   * case some instantiate the AsposeDocFactory and not through
   * {@link #getInstance()}.<br>
   ...
   */
  public <T> BaseDocFactory withDocumentWorker(T worker) {
   ...
   */
  public abstract BaseDocFactory withSimpleMergeCleanupOption(
          SimpleMergeCleanupOptions simpleMergeCleanupOption);
  ...
  public BaseDocFactory withDocumentCreationOptions(DocumentCreationOptions options) {
