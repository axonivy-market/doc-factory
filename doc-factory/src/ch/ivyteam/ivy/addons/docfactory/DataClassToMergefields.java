...
/**
 * @author ec
 * @since 07/02/2011 Used to map Ivy DataClasses to usable Objects for Aspose
 *        mail merge or mail merge with regions.
 */
public class DataClassToMergefields {

  /**
   * Takes a CompositeObject (dataClass) returns a list of
   * TemplateMergeField<br>
   * that can be used to write a serial letter with a Template document.<br>
   * The names of the template Document should be the same as the fields of the
   * dataClass.
   * @param _data CompositeObject (dataClass)
   * @return list of TemplateMergeField that can be used to write a serial
   *         letter with a Template document.
   */
  public static List<TemplateMergeField> transformDataClassInMergeField(CompositeObject _data) {
    ...
    for (int i = 0; i < fields.length; i++) {
      try {
        TemplateMergeField tmp = new TemplateMergeField(fields[i].getName(),
                _data.get(fields[i].getName()).toString());
        tmps.add(tmp);
      } catch (Exception _ex) {
  ...
  public static Hashtable<String, Recordset> transformDataClassInTablesNamesAndFields(CompositeObject _data) {
    ...
    for (int i = 0; i < fields.length; i++) {
      try {
        if (fields[i].getType() == Recordset.class) {
          String fieldName = fields[i].getName();
          tablesNamesAndFieldsHashtable.put(fieldName, (Recordset) _data.get(fieldName));
        }
      } catch (Exception _ex) {
        // do nothing and go to the next field
      }
  ...
  public static IMailMergeDataSource transfomDataClassInMergeDataSource(
          java.util.List<CompositeObject> compositeObjectsList, String _tableName) throws Exception {
    ...
    for (int i = 1; i < compositeObjectsList.size(); i++) {
      ...
      // type
      if (compositeObjectsList.get(i).getClass().getName() != compositeObjectsList.get(i - 1).getClass()
              .getName()) {
    ...
    if (_tableName == null || _tableName.trim().equals("")) {
      mmds = new MailMergeDataSource(rs, compositeObjectsList.get(0).getClass().getName()
              .substring(compositeObjectsList.get(0).getClass().getName().lastIndexOf(".") + 1));
    } else {
