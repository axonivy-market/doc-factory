...
PrimeFaces.widget.Freya = PrimeFaces.widget.BaseWidget.extend({
    ...
    restoreMenuState: function() {
        ...
        if(sidebarCookie && !this.isHorizontal() && !this.isMobile()) {
...
PrimeFaces.FreyaConfigurator = {
    ...
    changeLayout: function( componentTheme, darkMode ) {
        ...
        var now = new Date();
        var expireTime = now.getTime() + 30*86400000; //30 days * 1 day in ms
        now.setTime(expireTime);
        $.cookie('primefaces-theme-mode', darkMode, {path: '/', expires: now});
        this.updateDialogFrames(componentTheme, darkMode);
    },

    updateDialogFrames: function(componentTheme, darkMode) {
        var frames = window.frames;
        for (var i = 0; i < frames.length; i++) {
            try {
                frames[i].frameElement.contentWindow.PrimeFaces.FreyaConfigurator.changeLayout(componentTheme, darkMode);
            } catch (e) {
                // Ignore error
            }
        }
    ...
    changeLayoutsTheme: function(darkMode) {
        newLayout = '-' + darkMode;
        var linkElement = $('link[href*="layout-ivy-"]');
        var href = linkElement.attr('href');
        var startIndexOf = href.indexOf('layout-ivy-') + 10;
        var endIndexOf = href.indexOf('.css');
        var currentColor = href.substring(startIndexOf, endIndexOf);
        this.replaceLink(linkElement, href.replace(currentColor, newLayout));
    },

    changeLogo: function(darkMode) {
        var logoPattern = darkMode === 'dark' ? 'logo.' : 'logo_light.';
        var newLogoPattern = darkMode === 'light' ? 'logo.' : 'logo_light.';
        var imgElements = $.makeArray($('img[src*="xv="]'));
        var logoElements = imgElements.filter(e => {
            return e.src.includes(logoPattern);
        });
        logoElements.forEach(logo => {
            var src = logo.src;
            this.replaceAttr($(logo), 'src', src.replace(logoPattern, newLogoPattern))
        })
    },
    ...
    replaceLink: function(linkElement, href) {
        this.replaceAttr(linkElement, 'href', href);
    },

    replaceAttr: function(element, name, value) {
        ...
        if (isIE) {
            ...
            var cloneElement = element.clone(false);

            cloneElement.attr(name, value);
            element.after(cloneElement);
            
            cloneElement.off('load').on('load', function() {
                element.remove();
            });
